...
foreach ( $all_questions_link as $key => $url ) {
	$safe_key = preg_replace('/[^a-zA-Z0-9._-]/', '_', $key);

	$question_image = $image_folder . '/question_' . $safe_key . '.png';
	$answer_image   = $image_folder . '/answer_' . $safe_key . '.png';

	$escaped_question_image = escapeshellarg( $question_image );
	$escaped_answer_image   = escapeshellarg( $answer_image );
	$escaped_url            = escapeshellarg( $url );

	$output = shell_exec( "node capture.js $escaped_url $escaped_question_image $escaped_answer_image" );
}
