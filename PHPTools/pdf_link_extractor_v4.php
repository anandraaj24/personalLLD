...
<?php
/**
 * Automatically detects chapter number (e.g., 8) from first occurrence of X.x.x
 * Then extracts all section numbers like X.x.x and maps to GateOverflow links.
 *
 * Usage: php auto_detect_chapter_sections.php /path/to/file.pdf [output.json]
 */
...
$outPath = $argc >= 3 
    ? $argv[2] 
    : preg_replace('/\.pdf$/i', '', $pdfPath) . '_section_map.json';
...
// 🔍 Auto-detect chapter number from first match of X.x.x
function detect_chapter_number($text) {
    // Match any section number like 8.1.2, 5.3.4, etc.
    if (preg_match('/\b(\d+)\.\d+\.\d+\b/', $text, $match)) {
        return $match[1]; // returns '8' from '8.1.2'
    }
    return null;
}

// ✅ Extract all section numbers matching <chapter>.x.y
function extract_section_numbers($text, $chapter) {
    $pattern = '/\b' . preg_quote($chapter, '/') . '\.\d+\.\d+\b/';
...
// Step 1: Auto-detect chapter number
$chapter = detect_chapter_number($text);

if (!$chapter) {
    fwrite(STDERR, "Error: Could not detect chapter number (e.g., 8.x.x) in the PDF.\n");
    exit(1);
}

echo "Detected chapter: $chapter\n";

// Step 2: Extract all section numbers like <chapter>.x.y
$numbers = extract_section_numbers($text, $chapter);

// Step 3: Extract GateOverflow links
...
echo "Extracted " . count($numbers) . " section numbers ({$chapter}.x.y format).\n";
echo "Found " . count($links) . " GateOverflow links.\n";
echo "Mapped " . count($mapping) . " entries.\n";
